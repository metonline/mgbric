            ...
            for row in rows:
                ...
                if len(cells) >= 3 and current_direction:
                    try:
                        ...
                        # Extract pair number from row onclick attribute
                        # Format: onclick="location.href='pairsummary.php?event=404155&section=A&pair=9&direction=NS';"
                        pair_num = None
                        onclick = row.get('onclick', '')
                        if 'pair=' in onclick:
                            import re
                            pair_match = re.search(r'pair=(\d+)', onclick)
                            if pair_match:
