#!/usr/bin/env python3
"""
Enhanced scraper: Extract suit symbols for contracts.
Parse the contract row to get: level + suit + declarer + result
"""
...
def extract_board1_with_suit(driver, pair_num, direction, board_num=1):
    """Extract Board 1 data with suit symbols for contract."""
    try:
        time.sleep(0.5)
                        ...
                        try:
                            ...
                            if fantastic_rows:
                                ...
                                if len(cells) >= 3:
                                    ...
                                    # Extract contract level and suit
                                    contract_text = contract_cell.text.strip()
                                    
                                    # Get suit from img alt text
                                    suit_img = contract_cell.find_elements(By.TAG_NAME, "img")
                                    suit = ""
                                    if suit_img:
                                        try:
                                            alt_text = suit_img[0].get_attribute("alt").strip().upper()
                                            if alt_text in SUITS:
                                                suit = SUITS[alt_text]
                                            elif alt_text == "NT":
                                                suit = "NT"
                                        except:
                                    ...
                                    # Build contract string: level + suit + declarer
                                    if contract_text and declarer:
                                        if suit:
                                            contract = f"{contract_text}{suit} ({declarer})"
                                        else:
...
def main():
    ...
    try:
        ...
        print(f"Extracting: Contracts with suit symbols\n")
        ...
        print(f"\nFetching Board {BOARD_NUM} data with suit symbols for {total} pairs...")
        print("=" * 90)
        ...
        for i, pair in enumerate(ns_pairs):
            ...
            data = extract_board1_with_suit(driver, pair['pair_num'], pair['direction'], BOARD_NUM)
        ...
        # Save results
        output_file = "board1_with_suits.json"
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(all_results, f, indent=2, ensure_ascii=False)
        
        print(f"\n✓ Saved Board {BOARD_NUM} data with suit symbols to {output_file}")
        
        # Show sample
        print("\nSample results (with suit symbols):")
